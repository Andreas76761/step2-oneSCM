...
- **Git-Quellverbindungen** (`source_connections`): Repository-URL (nur `https`), Branch, Unterordner, Intervall (0 = manuell, 5 … 10080 Minuten).
  ...
  - Unveränderter Commit → kein Import; sonst ZIP der Dateien unterhalb des Unterordners durch die normale Pipeline (Pfade relativ zum Unterordner). Gelöschte Dateien bleiben wie bei ZIP-Importen als Quelle erhalten.
