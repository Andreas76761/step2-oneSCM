...
import { ensureDigestJob, runDigests } from './services/digest.js';
...
export async function buildApp(overrides: Partial<AppConfig> = {}, options: BuildOptions = {}) {
  ...
  jobs.register('weekly-digest', async () => {
    await runDigests(ctx, (id) => withProject(ctx, id));
    await ensureDigestJob(ctx, true);
  }, undefined, { background: true });
