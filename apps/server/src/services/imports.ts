...
export async function createImport(ctx: Ctx, fileName: string, data: Buffer, actor: string) {
  const settings = getSettings(ctx.db).import;
  ...
  const storageKey = `uploads/${hash}`;
  await ctx.store.put(storageKey, data);
  const id = newId('imp');
  ctx.db.run(
  ...
  audit(ctx.db, actor, 'import.created', 'import', id, { fileName, sha256: hash });
  ctx.jobs.enqueue(`import:${id}`, () => processImport(ctx, id, fileName, data));
...
export async function processImport(ctx: Ctx, importId: string, fileName: string, data: Buffer) {
  const { db } = ctx;
  db.run("UPDATE imports SET status = 'processing' WHERE id = ?", importId);
  const stats = { files: 0, imported: 0, identical: 0, failed: 0, skipped: 0, snippets: 0 };
  let entries: Entry[];
  try {
    entries = await readEntries(ctx, fileName, data);
  } catch (e) {
    db.run("UPDATE imports SET status = 'failed', error = ?, finished_at = ?, stats = ? WHERE id = ?", (e as Error).message, now(), json(stats), importId);
    return;
  }
  ...
  for (const entry of entries) {
    ...
    db.run('INSERT INTO import_items (id, import_id, path, sha256, status, message, revision_id) VALUES (?, ?, ?, ?, ?, ?, ?)', item.id, importId, item.path, item.sha256, item.status, item.message, item.revisionId);
  }

  const status = stats.failed === 0 ? 'completed' : stats.failed === stats.files ? 'failed' : 'completed_with_errors';
  db.run('UPDATE imports SET status = ?, finished_at = ?, stats = ? WHERE id = ?', status, now(), json(stats), importId);
  audit(db, 'system', 'import.finished', 'import', importId, { status, ...stats });
}

async function storeRevision(ctx: Ctx, importId: string, filePath: string, data: Buffer, text: string) {
  const { db } = ctx;
  const hash = sha256(data);
  let doc = db.get('SELECT * FROM source_documents WHERE project_id = ? AND path = ?', ctx.projectId, filePath);
  const latest = doc ? db.get('SELECT * FROM source_revisions WHERE document_id = ? ORDER BY revision_no DESC LIMIT 1', doc.id) : undefined;
  if (latest && latest.sha256 === hash) return { identical: true, revisionId: latest.id as string, revisionNo: latest.revision_no as number, snippets: 0 };
  ...
  db.tx(() => {
    if (!doc) {
      doc = { id: newId('doc') };
      db.run('INSERT INTO source_documents (id, project_id, path, file_name, created_at) VALUES (?, ?, ?, ?, ?)', doc.id, ctx.projectId, filePath, path.basename(filePath), now());
    }
    db.run('UPDATE source_revisions SET is_current = 0 WHERE document_id = ?', doc.id);
    db.run(
    ...
    for (const h of parsed.headings) {
      if (h.level === 1) ensureChapter(ctx, h.title);
      if (h.level === 2) ensureSubchapter(ctx, ensureChapter(ctx, h.chapterTitle).id, h.title);
    }

    for (const b of parsed.blocks) {
      const chapter = ensureChapter(ctx, b.chapterTitle);
      const sub = b.subchapterTitle ? ensureSubchapter(ctx, chapter.id, b.subchapterTitle) : null;
      ...
      db.run(
        ...
        snippetId, db.nextSeq('text_snippets'), revisionId, chapter.id, sub?.id ?? null, json(b.headingPath), b.position, b.lineStart, b.lineEnd, b.kind, b.text,
      ...
      for (const r of cls.roles) db.run('INSERT INTO snippet_roles VALUES (?, ?, ?, ?, ?, ?, ?)', snippetId, r.code, r.score, r.method, r.modelVersion, r.evidenceStatus, r.evidence);
      for (const d of cls.divisions) db.run('INSERT INTO snippet_divisions VALUES (?, ?, ?, ?, ?, ?, ?)', snippetId, d.code, d.score, d.method, d.modelVersion, d.evidenceStatus, d.evidence);
      if (cls.market) db.run('INSERT OR IGNORE INTO markets (code, label) VALUES (?, ?)', cls.market.code, cls.market.code);
      if (cls.release) db.run('INSERT OR IGNORE INTO release_scopes (code, label) VALUES (?, ?)', cls.release.code, cls.release.code);
      snippetCount++;
    }
  });
  return { identical: false, revisionId, revisionNo, snippets: snippetCount, warning: parsed.frontMatterError ? `Front-Matter ungültig: ${parsed.frontMatterError}` : undefined };
}
...
function ensureChapter(ctx: Ctx, title: string | null): Row {
  const { key, title: display } = chapterOf({ chapterTitle: title });
  const existing = ctx.db.get('SELECT * FROM chapters WHERE project_id = ? AND key = ?', ctx.projectId, key);
  if (existing) return existing;
  const pos = numberedPosition(display) ?? (ctx.db.get<{ m: number | null }>('SELECT MAX(position) m FROM chapters WHERE project_id = ?', ctx.projectId)?.m ?? 0) + 1;
  const row = { id: newId('ch'), key, title: display, position: key === '__none__' ? 0 : pos };
  ctx.db.run('INSERT INTO chapters (id, project_id, key, title, position) VALUES (?, ?, ?, ?, ?)', row.id, ctx.projectId, row.key, row.title, row.position);
  return row;
}

function ensureSubchapter(ctx: Ctx, chapterId: string, title: string): Row {
  const key = headingKey(title) || title;
  const existing = ctx.db.get('SELECT * FROM subchapters WHERE chapter_id = ? AND key = ?', chapterId, key);
  if (existing) return existing;
  const pos = numberedPosition(title) ?? (ctx.db.get<{ m: number | null }>('SELECT MAX(position) m FROM subchapters WHERE chapter_id = ?', chapterId)?.m ?? 0) + 1;
  const row = { id: newId('sub'), key, title, position: pos };
  ctx.db.run('INSERT INTO subchapters (id, chapter_id, key, title, position) VALUES (?, ?, ?, ?, ?)', row.id, chapterId, key, title, pos);
  return row;
}

export function getImport(ctx: Ctx, id: string) {
  const row = ctx.db.get('SELECT * FROM imports WHERE id = ?', id);
  if (!row) throw notFound(`Import ${id}`);
  const items = ctx.db.all('SELECT id, path, sha256, status, message, revision_id AS revisionId FROM import_items WHERE import_id = ? ORDER BY rowid', id);
  return {
...
export function listImports(ctx: Ctx) {
  return ctx.db.all('SELECT id FROM imports WHERE project_id = ? ORDER BY created_at DESC', ctx.projectId).map((r) => {
    const { items, ...rest } = getImport(ctx, r.id);
    return { ...rest, itemCount: items.length };
  });
}

export function listSources(ctx: Ctx) {
  const docs = ctx.db.all('SELECT * FROM source_documents WHERE project_id = ? ORDER BY path', ctx.projectId);
  return docs.map((d) => ({
    id: d.id,
    path: d.path,
    fileName: d.file_name,
    revisions: ctx.db.all(
...
export async function getRevisionRaw(ctx: Ctx, revisionId: string) {
  const r = ctx.db.get('SELECT storage_key FROM source_revisions WHERE id = ?', revisionId);
